...
function EditableChurchInfoTable() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('https://api.chriscocentralnairobi.org/churchinfo/all');
  ...
  const handleSave = async () => {
    try {
      await axios.put(`https://api.chriscocentralnairobi.org/churchinfo/update/1`, churchInfo, config);
